...
export default class Activities {
    ...
    doReadToEarn = async (accessToken: string, data: DashboardData): Promise<{ articlesRead: number, totalPointsGained: number }> => {
        const readToEarn = new ReadToEarn(this.bot)
        return await readToEarn.doReadToEarn(accessToken, data)
    }

    doDailyCheckIn = async (accessToken: string, data: DashboardData): Promise<{ success: boolean, pointsGained: number, message: string }> => {
        const dailyCheckIn = new DailyCheckIn(this.bot)
        return await dailyCheckIn.doDailyCheckIn(accessToken, data)
    }
